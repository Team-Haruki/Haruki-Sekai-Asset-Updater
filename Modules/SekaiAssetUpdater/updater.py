...
class SekaiAssetUpdater:
    def __init__(
        ...
        skip_download_prefix: Tuple,
        startapp_prefix: Tuple,
        ondemand_prefix: Tuple,
        asset_version: str,
        ...
        self.skip_download_prefix = skip_download_prefix
    ...
    async def _save_downloaded_assets(self, data: Union[Dict, List]) -> None:
        async with aiofiles.open(self.downloaded_assets_file, "wb") as f:
    ...
    async def init(self) -> None:
        ...
        if self.server == SekaiServerRegion.JP:
            profile = "production"
            self.asset_info_url = self.asset_info_url.format(
                env=profile, hash=get_environment(profile), asset_version=self.asset_version, asset_hash=self.asset_hash
            )
            self.asset_url = self.asset_url.format(
                env=profile, hash=get_environment(profile), asset_version=self.asset_version, asset_hash=self.asset_hash
            )
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            for _bundle, _bundle_info in _assetbundle_info["bundles"].items():
                if _bundle.startswith(self.skip_download_prefix):
                    continue
            ...
            with Progress() as _progress:
                _task_id = _progress.add_task(
                    f"Downloading {self.server.value.upper()} new assets...", total=len(_to_download_list)
                )
                _download_tasks = [
                    self.download_asset(_semaphore, bundle_path, bundle_hash, _progress, _task_id)
                    for bundle_path, bundle_hash in _to_download_list.items()
                ]
                _result = await asyncio.gather(*_download_tasks, return_exceptions=True)

        except Exception as e:
            logger.exception(f"Asset updater run() failed with unexpected error: {repr(e)}")
            ...
            for item in _result:
                if isinstance(item, tuple):
                    ...
                    if self.server in [SekaiServerRegion.TW, SekaiServerRegion.KR, SekaiServerRegion.CN]:
                        _result_filtered[_to_download_bundle_mapping.get(key)] = value
                    else:
                        _result_filtered[key] = value
                elif isinstance(item, Exception):
                    logger.error(f"Download task raised an exception: {repr(item)}")

            _downloaded_assets.update(_result_filtered)
